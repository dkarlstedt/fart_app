...
function initHandles() {
  state.handles = Array.from({ length: NUM_PETALS }, (_, i) => {
    // Normalize theta to [-π, π] so it matches Math.atan2 output range
...
function drawCrease(ctx, theta, dist, innerR) {
  ...
  // Teardrop: narrow at startR (inner), wide + rounded at dist (outer)
  ctx.beginPath();
...
function drawOpening(ctx, innerR) {
  // Soft ambient shadow pooling around the opening
...
function render() {
  ...
  // 1. Outer drop shadow (draw before skin so it shows outside the disc)
  ctx.save();
  ...
  // 3. Subtle top-left lighting to give it a 3-D roundness
  ...
  // 4. Crease shadows — the radiating dark fold valleys
...
// surfacePreset: future hook — e.g. { filterQ: 5, lfoRate: 8 } for chair/hot-tub
